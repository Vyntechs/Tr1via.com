...
## Goal
A written validation report proving — on a THROWAWAY Supabase copy (preview branch, NOT prod, NOT the first host's data) with `0014` applied — that every risk the local tests couldn't reach is closed, so merging #106 is near-zero risk.
...
**OUT (do not touch):** do NOT merge #106 (Brandon merges); do NOT apply `0013`/`0014` to PROD or validate against the live DB; do NOT touch the first host's data; do NOT change the fix unless validation finds a real defect (then STOP + get Brandon's OK before changing the approach).
...
## Risk flag
⚠️ Production database migration on **live-user (the first host's) leaderboard** data. Plan-first; applied only post-merge, away from the Wed 6/17 show. The change is a `create or replace view` — instant, no lock, no backfill, and reversible by re-applying the prior definition.
...
## Scope
**IN:** apply `supabase/migrations/0013_game_scores_per_game_isolation.sql` to prod; a READ-ONLY prod verification query against `game_scores`; run `tests/e2e/full-game.spec.ts` against prod; a sanity glance at the first host's most recent real night's leaderboard.
...
## Risk flag
⚠️ Applies a migration to PRODUCTION (the first host's live leaderboard data). `create or replace view` is instant / no-lock / reversible, but it's a prod write — needs Brandon's go at apply-time and must be far from the Wed 6/17 show.
