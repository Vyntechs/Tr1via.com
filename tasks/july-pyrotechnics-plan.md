...
- **HARD CONSTRAINTS (all phases):** No DB migration expected (broadcast is transport, not schema — a new BroadcastTag needs no schema change). Never merge/deploy during a live Wednesday show (Phases 2–3 touch the realtime path the first host's real shows run on). Respect `prefers-reduced-motion` + a phone perf budget everywhere.
