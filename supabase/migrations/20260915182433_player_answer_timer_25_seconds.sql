-- Keep the authoritative resilient answer window aligned with the 25-second
-- player and TV countdown. The two-second final window remains unchanged.
...
declare
  ...
  v_main_duration interval := interval '25 seconds';
  v_final_window_duration interval := interval '2 seconds';
  v_eligible integer;
...
begin
  ...
  insert into public.question_plays (
    ...
    'accepting', v_now, v_now + v_main_duration,
    v_now + v_main_duration + v_final_window_duration
  );
  ...
  insert into public.live_room_events (
    ...
    jsonb_build_object(
      ...
      'mainZeroAt', v_now + v_main_duration,
      'finalWindowEndsAt', v_now + v_main_duration + v_final_window_duration
    )
  );
