...
async function onboard(baseUrl, nightId, playerIx) {
  if (!nightId) return { cookie: "", playerId: "" };
  ...
  if (!deviceCookie) return { cookie: "", playerId: "" };
  // Capture the REAL player id so heartbeats are genuine DB writes (not 404s).
  let playerId = "";
  try {
  ...
  return { cookie: deviceCookie, playerId };
}
...
async function runPlayer({ baseUrl, code, nightId, seconds, playerIx, samples, readsOnly }) {
  ...
  const { cookie, playerId } = await onboard(baseUrl, nightId, playerIx);
  const auth = cookie ? { cookie } : {};
  const heartbeatUrl = `${baseUrl}/api/players/${playerId || `vp-${playerIx}`}/heartbeat`;
  let tick = 0;
  ...
  while (performance.now() < deadline) {
    ...
    // Real heartbeat WRITE (updates last_seen_at) — every ~4.5s like a real phone.
    if (tick % 3 === 0) {
      samples.push(
        await timedFetch("POST /players/:id/heartbeat", heartbeatUrl, { method: "POST", headers: { ...auth } }),
      );
    }
