...
**Heather (`heatherhmoore@yahoo.com`) goes live on tr1via.com Wednesday 2026-05-27.** Real paying patrons. **2 days out.**
...
| PR | What | Status |
|---|---|---|
| #27 | `fix(auth)`: when authed, `/login` shows "Go to your dashboard →" CTA instead of magic-link form (closes the "click Send sign-in link 5 times → email rate limit" loop Heather hit) | **merged** |
...
Validated end-to-end on preview (`tr1via-cy1kqzyfg…`): Brandon's email → instant `/host`. Heather's email → instant `/host` with her data (venue, room code, host_id). Bogus email → inline error. After sign-in, re-visiting `/login` shows the SignedInPanel (no form). `hasCheckEmail: false` everywhere.

---

## 🚨 P0 carry-over: clone Heather's WIP to her host_id

Heather did her Wednesday-night setup on **Brandon's founder host_id** before sign-out existed (pre-PR #25, before 17:29 UTC). She lost access to it the moment she signed in on her own account. She needs it back.
...
1. **Deep-copy** the night to Heather's host_id (`772f91c9-c7fc-424b-9429-207e4527cad1`) in one transaction:
   - New `nights` row (host_id = Heather, copy venue/theme/scheduled_at/etc., new id)
...
4. **Tell Brandon**: "Heather can log in now; her Wednesday setup is in her dashboard."
...
Heather will hit "I can't rename a locked category" on Wednesday if not shipped.
...
|---|---|---|
| Generated 3 short-lived magic-link URLs for Heather via Supabase admin SDK | Manual unblocking during the rate-limit incident; superseded by /host/admin tool | Self-expire in ~1hr; first one was consumed on use |
| **Nothing to her actual data** | — | — |
| Several validation Playwright sessions signed in as Heather on previews — may have created 2 small "Heather"-venue nights under her host_id (ids `30cbd106…`, `b37e7808…`) | Test data from validation flows | Safe to delete if cluttering; Heather's real Wednesday work lives on Brandon's host_id (see P0) |
...
- **Email-login bypass for Claude** — `/login` → any host email → instant sign-in. Use brandon@vyntechs.com for founder ops, heatherhmoore@yahoo.com to validate as her.
...
The most urgent move on a clean start with no bug report is **clone night `655995cb` to Heather's host_id** (§ "P0 carry-over" above). Brandon already confirmed scope; just do it carefully and verify.
