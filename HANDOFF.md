# TR1VIA — Handoff (end of session 22, 2026-05-27 late afternoon, the first host-tonight fixes shipped)
...
- **the first host goes live tonight (2026-05-27).** She runs on whatever's currently on `main`. All today's fixes ARE on main — both PR #52 (May/Storm) and PR #53 (chrome + best category) merged.
...
3. **AI generating wrong-answer questions** (PR #52 — `1b7ec24` + `7483fa4`)
   - **Symptom:** the first host texted Brandon: AI question said "most commonly harvested tree" with Eucalyptus marked correct; Pine is at least as defensible.
   ...
   - **Root cause #2 (downstream affordance):** the host's "mark correct" button in `HostGenEdit` was 9px monospace muted — the first host couldn't find it. She knew the answer was wrong but didn't know how to fix it.
...
### Architectural follow-ups identified this session (not blocking the first host)
