...
7. **Test isolation discipline:** every test user email MUST end in `@tr1via.test`. `isTestEmail()` in `lib/api/require-test-mode.ts` is the allowlist. Brandon's `host@example.com` is structurally incapable of being touched by `/api/_test/reset`.
