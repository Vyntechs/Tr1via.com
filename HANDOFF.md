...
- **Heather goes live tonight (2026-05-27).** She'll be running on whatever's currently on `main`. The May/Storm feature is in PR #52, NOT yet merged.
...
4. **Test before pushing each fix.** TR1VIA's PR-first + Heather-live posture means broken intermediate states are unacceptable.
...
These remain on `fix-regenerate-picks-and-dups` (separate branch from this PR's work). They were stashed multiple times during session 21 execution but not touched. Both unblocked Heather's current-night experience because they're on a different branch.
...
- Heather can stay on May/Storm to get the magic, or pick any other theme.
