...
begin;
select public.reset_night_to_setup('52feb7b4-3e3a-4286-839e-74bceea030f0'::uuid);
-- Verify state changes
select id, state, started_at, ended_at from games where night_id = '52feb7b4-3e3a-4286-839e-74bceea030f0';
select opened_at from nights where id = '52feb7b4-3e3a-4286-839e-74bceea030f0';
select count(*) from reveals where game_id in (select id from games where night_id = '52feb7b4-3e3a-4286-839e-74bceea030f0');
select count(*) from answers a join questions q on q.id = a.question_id join categories c on c.id = q.category_id join games g on g.id = c.game_id where g.night_id = '52feb7b4-3e3a-4286-839e-74bceea030f0';
rollback;
```
...
```sql
select id, state from games where night_id = '52feb7b4-3e3a-4286-839e-74bceea030f0' order by game_no;
select opened_at from nights where id = '52feb7b4-3e3a-4286-839e-74bceea030f0';
```
...
Use Playwright MCP against `$PREVIEW`. The test night is `52feb7b4-3e3a-4286-839e-74bceea030f0` (Heather's `YU5JF3`). Sign in as Heather (`heatherhmoore@yahoo.com`) using `/auth/grant?t=` (founder-grant from Brandon's account if needed; see `project_auth_model_type_email_in.md`).
...
3. Tap Cancel → modal closes, dashboard unchanged. Verify SQL: `select opened_at, closed_at from nights where id = '52feb7b4-...'` → `opened_at` still set, `closed_at` still null. Capture `validate-03-cancel.png`.
...
```sql
select id, state, started_at, ended_at from games where night_id = '52feb7b4-3e3a-4286-839e-74bceea030f0' order by game_no;
select opened_at, closed_at from nights where id = '52feb7b4-3e3a-4286-839e-74bceea030f0';
select count(*) from reveals where game_id in (select id from games where night_id = '52feb7b4-3e3a-4286-839e-74bceea030f0');
select count(*) from answers a join questions q on q.id = a.question_id join categories c on c.id = q.category_id join games g on g.id = c.game_id where g.night_id = '52feb7b4-3e3a-4286-839e-74bceea030f0';
select count(*) from questions q join categories c on c.id = q.category_id join games g on g.id = c.game_id where g.night_id = '52feb7b4-3e3a-4286-839e-74bceea030f0' and q.finished_at is not null;
select count(*) from categories c join games g on g.id = c.game_id where g.night_id = '52feb7b4-3e3a-4286-839e-74bceea030f0';
select count(*) from questions q join categories c on c.id = q.category_id join games g on g.id = c.game_id where g.night_id = '52feb7b4-3e3a-4286-839e-74bceea030f0' and q.is_picked = true;
select count(*) from players where night_id = '52feb7b4-3e3a-4286-839e-74bceea030f0' and removed_at is null;
```
...
From the dashboard after reset, tap "Continue setup" → land in `/host/setup/52feb7b4-...`. Verify the existing setup UI loads with the original 6 Game 1 categories visible and editable.

- [ ] **Step 8: Verify player phone sees the reset (Realtime)**

In a second browser tab, open `$PREVIEW/r/YU5JF3` as a player (use one of the 4 existing test players' device id from the players table). Verify the phone shows the lobby/waiting screen, not the live game state. If any of the 4 players are still showing live state, that's a Realtime regression and we STOP.
...
**Preview URL:** $PREVIEW
**Validated against:** night 52feb7b4-3e3a-4286-839e-74bceea030f0 (YU5JF3)
**Validated by:** Claude
...
Triggered by Brandon hitting this exact trap on Heather's account the night before her go-live: 6 ready categories, 21 picked questions, 9 played, 25 answers from 4 friends — and the only way to "redo" was throw the whole night away.
...
- **Owner-only.** Same ownership check as `/open` and `/close`. Heather can't reset Brandon's nights.
- **Draft games untouched.** Only games that have been started or finished get reset. If Heather has Game 2 half-built, it stays half-built.
...
Wait ~1–2 min for Vercel prod deploy. Then sign in as Heather on `tr1via.com` and confirm the "Reset and edit game" button is visible on her dashboard (the test night is now in setup state from Task 8's validation; the button will be hidden until Heather opens a new night, but the dashboard layout should otherwise be normal).
...
```
Done. The Reset and edit game button is live on tr1via.com. Heather can use it. Full validation log + screenshots saved locally (not committed). Wednesday go-live is ~XYZ hours out — feature shipped, no gaps.
```
...
- Hiding the button behind a "..." overflow menu. Premature; revisit only if Heather complains about clutter.
