...
**Goal:** When a self-serve host's 30-day free trial ends, they can pay $4.99/mo via Stripe Checkout and keep generating trivia with AI — cancelling turns it back off. Founder + comped (the first host) hosts are never affected.
...
`founder` → ✓ · `is_paywall_bypassed` (comped) → ✓ · `subscription_status ∈ {active, trialing, past_due}` → ✓ · `trial_ends_at` in future → ✓ · else → **deny**. `past_due` stays on for Stripe's dunning grace; `canceled`/`unpaid`/NULL do not. **The founder + comp checks run first and are never replaced — the first host can't be locked out.**
...
4. **Manual test-mode E2E:** sign in as a non-comped host → "Upgrade" → pay `4242 4242 4242 4242` → confirm the webhook set `subscription_status='active'` → generate AI succeeds → cancel in portal → status flips → generate 402s. Founder + the first host generate throughout.
