...
- `components/marketing/Proof.tsx` — anonymized real-quote + honest-signal block (quote behind a flag until the first host's words arrive).
...
## Phase B — Build infrastructure (the the first host-safe theming engine)
...
describe("Proof", () => {
  ...
  it("renders the real quote with anonymous attribution when provided", () => {
    ...
    expect(getByTestId("proof-quote").textContent).not.toMatch(/the first host/i); // never the real name
  });
});
```

- [ ] **Step 2: Run** — FAIL. **Step 3:** Implement `Proof` so `quote` is an optional prop; with `null` it renders only honest signal ("live trivia nights running weekly", the anti-cheat guarantee, free-forever); with a quote it renders `data-testid="proof-quote"` + the anonymous attribution. The hub passes `quote={null}` until Brandon supplies the first host's real words. **Step 4: Run** — PASS. **Step 5:** commit `feat(marketing): Proof — honest signal now, real anonymized quote when available`.
...
- [ ] **Step 5: Open PR** to `main` (Brandon merges; never push to `main` directly). PR body lists the the first host-safe guarantee + the `git diff --name-only` proof that only marketing files changed.

---

## Open items / blocking
- **the first host's real quote** blocks ONLY the Proof quote (Task 8 ships honest-signal-only until then). Everything else proceeds.
- **Section→month map** (Task A3 table) is the current proposal; finalize visually at GATE A3.

## Self-review notes
- Spec §2 (the first host-safe) → Task 0 + V3 scope guard. Spec §3 (segments) → Task 5. Spec §5 (Year Scroll + SSR-first + reduced-motion) → Tasks 1–3, V2. Spec §6 (honest proof) → Task 8. Spec §7 (spine) → Tasks 4–7. Spec §4 (/pricing, /themes) → Tasks 9–10. Spec §10 (verification) → V1–V3. All spec sections have a task.
