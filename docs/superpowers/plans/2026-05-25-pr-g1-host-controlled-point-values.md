...
**Goal:** Make `questions.point_value` (the 100/200/.../700 slot on the board) directly host-controlled via the Edit panel, instead of an opaque derivation from Claude-rated `difficulty`. the first host's edits to "this should be the 400-pointer" must actually land at 400.
...
gh pr create --title "feat(setup): host-controlled point values (fixes the first host #1 + #4)" --body "..."
```

PR body should include:
- the first host's quote (the specific complaints)
...
**Spec coverage:**
- the first host #1 (order on board) → addressed by Task 7 (board respects pointValue) + Tasks 1, 5 (host places explicitly)
- the first host #4 (Edit doesn't change board) → root-cause fix via Tasks 1-8 collectively
- the first host #2 (write own question) → out of scope for G1; G3 territory
- the first host #3 (rename category) → out of scope for G1; G2 territory
- the first host #5 (date vs time) → deferred indefinitely (confirmed)
