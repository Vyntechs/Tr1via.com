...
**Customer trigger:** Host Heather — "lock three of the questions in… I wanted to do another 20 but I don't wanna lose the ones I have."
...
- **The "regenerate wipes my picks" bug is already fixed at root.** Picks live in client
  ...
  **Heather's literal "keep my 3, do another 20" already works.** No fix needed for that part.
...
- **Cross-session/whole-history dedup** — we avoid repeating the questions currently on screen
  ...
  many rerolls. YAGNI; revisit only if Heather reports repeats across multiple rerolls.

## 3. Behavior spec (Heather's flow)
