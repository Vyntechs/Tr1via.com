...
- **Heather-safe:** `git diff` touches only `app/(marketing)/**`, `app/page.tsx`, `components/marketing/**`, and the new spec/plan docs. Nothing under host/player/tv/api/lib-theme/supabase changes. (Hard gate, checked before any PR.)
...
- **Honesty:** no testimonial/quote ships without Heather's real (anonymized) words.

## 11. Risks / open items
- **Open (blocking the Proof section only):** Heather's real quote. Everything else can proceed; Proof ships last.
