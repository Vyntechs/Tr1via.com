...
At Heather's show the host laptop went to sleep, and when it woke the live connection became a "dead line that still looks connected" — both ends thought they were fine, but nothing got through. The screen froze for 3.5 minutes (proven in the data: a 215-second gap where no host click reached the server). The only thing that fixed it was fully quitting and reopening Chrome, which forced a brand-new connection.
