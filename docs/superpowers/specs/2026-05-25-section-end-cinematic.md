...
The "Pick the next topic" panel currently shown after a category's last question takes the first host's question-picking control away. After tapping a topic, the system auto-starts that topic's lowest-points unplayed question — she can't pick by point value. the first host is going to feel this and say something on Wednesday.
...
the first host picks **any remaining question by any remaining point value** at all times — same Jeopardy grid she's used to. *And* the moment a topic finishes, the room gets a cinematic "Section Complete" beat that feels earned. Both. No half-measures.
...
Add `TVSectionComplete` to `/dev/tv` gallery with `staticHold={true}` so the frame can be screenshotted for marketing or the first host walkthroughs.
